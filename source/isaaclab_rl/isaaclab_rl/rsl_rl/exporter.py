...
class _TorchPolicyExporter(torch.nn.Module):
    ...
    def forward(self, x):
    #     if self.estimator_enable:
    #         lantent = self.estimator(self.normalizer(x))
    #         lantent = lantent[:,:19]
    #         return self.actor(torch.cat((x[:,self.one_step_idx],lantent), dim=-1))
    #     else:
    #         return self.actor(self.normalizer(x))
        if self.estimator_enable:
            return self.policy.dwaq_inference(self.normalizer(x))
        else:
            return self.actor(self.normalizer(x))
...
class _OnnxPolicyExporter(torch.nn.Module):
    ...
    def __init__(self, policy, normalizer=None, verbose=False):
        ...
        if hasattr(policy, "estimator"):
            self.policy = copy.deepcopy(policy)
            # self.actor = copy.deepcopy(policy.actor)
            self.actor = copy.deepcopy(policy.estimator)
            self.estimator_enable = True
        elif hasattr(policy, "actor"):
            self.actor = copy.deepcopy(policy.actor)
            if self.is_recurrent:
